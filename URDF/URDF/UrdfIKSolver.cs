...
public class UrdfIKSolver : MonoBehaviour
{
    ...
    private IKResult SolveIK(string endEffectorName, Vector3 targetPosition, Quaternion? targetRotation)
    {
        ...
        var jointChain = robot.GetControllableJointChain(endEffectorName);
        if (jointChain.Count == 0)
        {
            Debug.LogWarning($"No controllable joints found in chain to {endEffectorName}");
            return new IKResult(false, new float[0], 0, float.MaxValue, float.MaxValue);
        }
        
        // Filter to revolute joints only
        jointChain = jointChain.Where(j => j is UrdfJointRevolute).ToList();
        if (jointChain.Count == 0)
        {
            Debug.LogWarning("No revolute joints in chain");
            return new IKResult(false, new float[0], 0, float.MaxValue, float.MaxValue);
        }
        ...
        if (useSolutionCaching && !HasTargetChanged(endEffectorName, targetPosition, targetRotation))
        {
            ...
            {
                // Verify the cached solution is still valid
                float[] cachedSolution = lastSolutions[endEffectorName];
                robot.SetJointPositions(jointChain, cachedSolution);
                
                Vector3 currPos = endEffectorLink.transform.position;
                float posErr = (targetPosition - currPos).magnitude;

                float rotErr = 0f;
                if (targetRotation.HasValue)
                {
                    Quaternion currRot = endEffectorLink.transform.rotation;
                    rotErr = Quaternion.Angle(targetRotation.Value, currRot) * Mathf.Deg2Rad;
                }
        ...
        // Check if we're already at the target (early exit)
        robot.SetJointPositions(jointChain, startPositions);
        Vector3 currentPos = endEffectorLink.transform.position;
        Quaternion currentRot = endEffectorLink.transform.rotation;
        ...
        {
            // Apply current positions to evaluate forward kinematics
            robot.SetJointPositions(jointChain, resultPositions);
            
            // Get current end effector pose
            currentPos = endEffectorLink.transform.position;
            currentRot = endEffectorLink.transform.rotation;
            ...
            float[,] J = ComputeJacobian(jointChain, endEffectorLink.transform.position, taskDim);
    ...
    /// </summary>
    private float[,] ComputeJacobian(List<UrdfJoint> jointChain, Vector3 endEffectorPos, int taskDim)
    {
        int dof = jointChain.Count;
        ...
        for (int i = 0; i < dof; i++)
        {
            ...
            Vector3 jointAxis = joint.WorldAxis;
            
            // For revolute joints:
            // Linear velocity contribution: axis × (end_effector - joint_position)
            Vector3 linearVel = Vector3.Cross(jointAxis, endEffectorPos - jointPos);
            
            J[0, i] = linearVel.x;
            J[1, i] = linearVel.y;
            J[2, i] = linearVel.z;
            
            // Angular velocity contribution: axis
            if (taskDim == 6)
            {
                J[3, i] = jointAxis.x;
                J[4, i] = jointAxis.y;
    ...
    public bool SolveAndApplyIK(string endEffectorName, Vector3 targetPosition, Quaternion? targetRotation = null)
    {
        ...
        {
            var jointChain = robot.GetControllableJointChain(endEffectorName)
                .Where(j => j is UrdfJointRevolute)
                .ToList();
            robot.SetJointPositions(jointChain, result.jointPositions);
